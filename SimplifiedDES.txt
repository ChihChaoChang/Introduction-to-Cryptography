...
def EPinv(X): 
    r""" 
    a)Consider EP, the expansion permutation, find an inverse contraction permutation that takes 8
    bits down to 4 and inverts EP. Note that these compressions permutations are not unique.
    """
...
def f_K_NoSBox(block, K):  
    r"""
    b)Take the function f_K from the example Sage code and modify it so that instead of calling the
    SBoxes, it calls EPinv after the round key is XORed in.
    """
    left_block = LeftHalfBits(block);
    right_block = RightHalfBits(block);
    temp_block1 = EP(right_block);    
...
def SDESEncrypt_NoSBox(plaintext_block, K):  
    ...
    Modify the functions SDESEncrypt and SDESDecrypt as necessary so that they call f_K_NoSBox.
    Call the new functions SDESEncryptNoSBox and SDESDecryptNoSBox.
    Peforms a single SDES plaintext block encryption.
    (Given plaintext and key as bit lists.)
    """
    (K1, K2) = SDESKeySchedule(K);
...
def SDESDecrypt_NoSBox(ciphertext_block, K):  
    r"""
    Performs a single SDES ciphertext block decryption.
    (Given ciphertext and key as bit lists.)
    """
...
P0=[1,0,1,0,0,0,1,1];
P1=[0,0,0,1,0,0,1,1];
K=[1,1,1,0,0,1,1,0,1,1];
A=SDESEncrypt_NoSBox(P0,K);
B=SDESEncrypt_NoSBox(P1,K);
A;
[1, 0, 1, 0, 1, 1, 0, 0]
B;
[1, 0, 1, 0, 1, 0, 1, 0]

SDESDecrypt_NoSBox(A,K);
[1, 0, 1, 0, 0, 0, 1, 1]

SDESDecrypt_NoSBox(B,K);
[0, 0, 0, 1, 0, 0, 1, 1]
